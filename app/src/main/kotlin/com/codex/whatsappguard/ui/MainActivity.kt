package com.codex.whatsappguard.ui
...
import com.codex.whatsappguard.AdminReceiver
import com.codex.whatsappguard.GuardAccessibilityService
...
open class MainActivity : Activity() {
    ...
    private fun buildContent(): View {
        ...
        root.addView(primaryButton(R.string.open_accessibility_settings) { openAccessibilitySettingsWithDisclosure() })
        root.addView(primaryButton(R.string.enable_device_admin)          { requestDeviceAdmin() })
        root.addView(primaryButton(R.string.apply_managed_protection)     { applyDeviceOwnerProtections() })
    ...
    private fun updateStatus() {
        ...
        statusText.text = buildString {
            appendLine(getString(if (accessibilityEnabled) R.string.status_accessibility_enabled  else R.string.status_accessibility_disabled))
            appendLine(getString(if (adminActive)          R.string.status_device_admin_enabled    else R.string.status_device_admin_disabled))
            append(getString(    if (deviceOwner)          R.string.status_managed_enabled         else R.string.status_managed_disabled))
        }
    }
    ...
    private fun applyDeviceOwnerProtections() {
        if (!devicePolicyManager.isDeviceOwnerApp(packageName)) {
            Toast.makeText(this, getString(R.string.managed_protection_requires_owner), Toast.LENGTH_LONG).show()
            updateStatus()
            return
        }
        ...
        runCatching {
            ...
            Toast.makeText(this, getString(R.string.managed_protection_error, it.message.orEmpty()), Toast.LENGTH_LONG).show()
        }
        updateStatus()
    }
